...
class Moderation(commands.Cog):
    ...
    async def unban(self, ctx: commands.Context, id: int):
        ...
        if id is None:
            ...
            try:
                ...
                await ctx.send(f"Failed trying to unban `{user}`. This user is probably already unbanned.")
    ...
    async def mute(self, ctx: commands.Context, member: discord.Member, *, reason: str = None):
        ...
        if not get(ctx.guild.roles, name="Himeji-Mute"):
            role = await ctx.guild.create_role(name="Himeji-Mute", permissions=discord.Permissions(send_messages=False))
