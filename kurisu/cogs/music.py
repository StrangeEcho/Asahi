...
class Music(commands.Cog):
    ...
    def __init__(self, bot: KurisuBot):
        ...
        self.spot_cid = self.bot.get_config("config", "music", "spot_client_id")
        self.spot_cs = self.bot.get_config("config", "music", "spot_client_sec")
    ...
    async def join(self, ctx: KurisuContext):
        ...
        if not ctx.author.voice:
            return await ctx.send_error("You are currently not in a Voice Channel")
    ...
    async def play(self, ctx: KurisuContext, *, query: str):
        ...
        components: discord.ui.MessageComponents = discord.ui.MessageComponents(
